...
export function AuctionsClient({ initialAuctions }: AuctionsClientProps) {
    const [search, setSearch] = useState("")
    const [statusFilter, setStatusFilter] = useState("all")

    const filteredAuctions = useMemo(() => {
        return initialAuctions.filter((auction) => {
            const productName = auction.products?.name || "Unknown Product"
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="rounded-md border bg-white overflow-hidden">
                <Table>
                    <TableHeader>
                        <TableRow className="bg-muted/50 hover:bg-muted/50 text-nowrap">
                            ...
                            <TableHead className="font-semibold text-center">Total Bids</TableHead>
                        ...
                        {filteredAuctions.map((auction: any) => {
                            ...
                            return (
                                <TableRow key={auction.id} className="hover:bg-muted/30 transition-colors">
                                    ...
                                    <TableCell className="font-mono text-sm">LKR {auction.starting_bid.toLocaleString()}</TableCell>
                                    <TableCell className="font-mono text-sm font-semibold text-primary">LKR {currentBid.toLocaleString()}</TableCell>
